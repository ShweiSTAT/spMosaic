...
def domain_detection(
    ...
    ## select a gene set to proceed # Default to common SVGs
    ...
    unique_batches, batch_labels_int = np.unique(combined_df['sample_name'].values, return_inverse=True)

    input_dim = Gene_exp_data.shape[1]   # Number of genes
    embedding_dim = 32
    ...
    model_preTrain = train_autoencoder(Gene_exp_data, 
                                       batch_labels_int,
                                       input_dim,
                                       embedding_dim, 
                                       batch_num,
    ...
    dec_model, y_pred_init = train_DEC(Gene_exp_data, 
    ...
    wanted_spot_meat_cols = ["sample_name","row","col","sum_umi","DEC_init_cluster","spatial_cluster","spatial_cluster_refined"]
    ...
    h5ad_out.obsm["spmosaic_embd"] = spot_embd
